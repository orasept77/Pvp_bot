...
from handlers.psr.keybs.start_psr import cancel_psr_private_lobby_cb, cancel_psr_private_lobby_keyb, \
    cancel_psr_type_private_lobby_id_keyb, connect_private_psr_lobby_cb, create_private_psr_lobby_cb, \
    cancel_psr_random_keyb, cancel_psr_revansh_keyb, psr_revansh_keyb, start_psr_keyb, psr_cb, cancel_psr_randon_cb, \
    psr_revansh_cb, cancel_psr_revansh_cb
        ...
        if lobby:
            if lobby["is_started"] == True:
                ...
                if len(players) == lobby['user_count'] - 1:
                    ...
                    await create_psr(repo, lobby['rates_id'], lobby['user_count'], players, lobby['game_type_id'], message,
                                     2)
                else:
                    await repo.add_private_lobby_user(int(message.text), message.from_user.id)
                    await message.answer(text="Вы были добавлены в лобби. Ожидайте остальных игроков",
                                         reply_markup=cancel_psr_private_lobby_keyb(lobby['id']))
                await state.finish()
        else:
...
async def start_psr_revansh(call: CallbackQuery, callback_data: dict, ):
    ...
    if len(players) == lobby['user_count'] - 1:
        ...
        await call.message.edit_text(text=call.message.text,
                                     reply_markup=cancel_psr_revansh_keyb(int(callback_data['private_lobby_id'])))
    await conn.close()


@dp.callback_query_handler(cancel_psr_revansh_cb.filter(), state="*")
async def psr_cancel_revansh(call: CallbackQuery, callback_data: dict, state: FSMContext):
    conn = await create_conn()
    repo = PSRRepo(conn)
    await repo.delete_private_lobby_user(int(callback_data['private_lobby_id']), call.from_user.id)
    ...
    data = await state.get_data()
    await call.message.edit_text(text=call.message.text,
                                 reply_markup=psr_revansh_keyb(int(callback_data['private_lobby_id']), data.get('rates_id'), data.get('user_count'), data.get('game_type_id')))
...
async def start_psr_random(call: CallbackQuery, callback_data: dict, ):
    ...
    if len(players) >= int(callback_data['user_count']) - 1:
        ...
        await create_psr(repo, int(callback_data['rates_id']), int(callback_data['user_count']), players,
                         int(callback_data['game_type_id']), call.message, 1)
        await call.message.delete()
    else:
...
async def create_psr(repo: PSRRepo, rates_id, user_count, players, game_type_id, message: Message, lobby_type_id):
    ...
    for player in players:
        message = await message.bot.send_message(chat_id=player['id'],
                                                 text=players_text_list + "Раунд 1\nВыберите вариант",
                                                 reply_markup=draw(variants, game_id))
...
async def prs_variant_user(call: CallbackQuery, callback_data: dict, state: FSMContext):
    conn = await create_conn()
    repo = PSRRepo(conn)
            ...
            if answer:
                ...
                if len(answers) == len(round_users):
                    ...
                    for variant in variants:
                        ...
                        variants_models.append(Variant(variant['id'], variant['title'],
                                                       [Variant(bvariant['id'], bvariant['title'], []) for bvariant in
                                                        bvariants]))
                    ...
                    if len(winners) > 1:
                        for round_user in round_users:
                            if round_user['user_id'] in winners:
                                ...
                                msg = await call.bot.edit_message_text(chat_id=round_user['user_id'],
                                                                       message_id=round_user['message_id'],
                                                                       text=text + result_text,
                                                                       reply_markup=draw(variants, game['id']))
                                ...
                                await call.bot.edit_message_text(chat_id=round_user['user_id'],
                                                                 message_id=round_user['message_id'],
                                                                 text=text + result_text, reply_markup=psr_revansh_keyb(
                                        game['private_lobby_id'], data.get('rates_id'), data.get('user_count'), data.get('game_type_id')))
                    elif len(winners) == 1:
                        await repo.set_game_end(game['id'])
                        data_rate_id = await repo.get_rate_id(game['id'])
                        data_user_count = await repo.get_user_count(game['id'])
                        data_game_type_id = await repo.get_game_type_id(game['id'])
                        for round_user in round_users:
                            if round_user['user_id'] in winners:
                                await call.bot.edit_message_text(chat_id=round_user['user_id'],
                                                                 message_id=round_user['message_id'],
                                                                 text=text + "Вы победитель",
                                                                 reply_markup=psr_revansh_keyb(
                                                                     game['private_lobby_id'], rates_id=int(data_rate_id['rates_id']), user_count=int(data_user_count['user_count']), game_type_id=int(data_game_type_id['game_type_id'])))
                            else:
                                await call.bot.edit_message_text(chat_id=round_user['user_id'],
                                                                 message_id=round_user['message_id'],
