...
from handlers.psr.keybs.start_psr import cancel_type_private_lobby_id, connect_private_psr_lobby_cb, create_private_psr_lobby_cb, cancel_psr_random_keyb, cancel_psr_revansh_keyb, psr_revansh_keyb, start_psr_keyb, psr_cb, cancel_psr_randon_cb, psr_revansh_cb, cancel_psr_revansh_cb
...
async def connect_private_lobby(call:CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(text=text, reply_markup=cancel_type_private_lobby_id())
    return
    ...
    if message.text.isnumeric():
        ...
        if lobby:
            ...
            if len(players) >= lobby['user_count'] - 1:
                players = players[:lobby['user_count'] - 1]
                ...
                await create_psr(repo, lobby['rates_id'], lobby['user_count'], players, lobby['game_type_id'], message.from_user.id, message, 2)
            else:
                await message.answer(text="Вы были добавлены в лобби. Ожидайте остальных игроков", reply_markup=cancel_type_private_lobby_id())
        else:
...
async def psr_confirm_screen(call:CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if data.get('type') == 'random_player':
        ...
        await call.message.edit_text(text=text+"\nИгра создана.\nВы были добавлены в лобби игры. <b>Идентификатор игры: {}</b>".format(id), reply_markup=to_menu())
