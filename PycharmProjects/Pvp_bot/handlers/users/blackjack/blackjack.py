...
async def start_blackjack(game_id):
    ...
    for player in players:
        ...
        f"Игра начинается!\n"
        f"Игрок 1: {users_data[0][0][1]} - @{users_data[0][0][3]}\n"
        f"Игрок 2: {users_data[1][0][1]} - @{users_data[1][0][3]}\n")
...
async def blackjack_endgame(game_id):
    ...
    if result == 'player_one_won':
        result = f'Победа игрока {users_data[0][0][1]} - @{users_data[0][0][3]}'
        await deposit_repo.plus_user_deposit(users_data[0][0][0], rate[0])
        await deposit_repo.minus_user_deposit(users_data[1][0][0], rate[0])
        await stat_repo.update_win_blackjack(users_data[0][0][0])
        await stat_repo.update_games_blackjack(users_data[0][0][0])
        await stat_repo.update_games_blackjack(users_data[1][0][0])
    elif result == 'player_two_won':
        result = f'Победа игрока {users_data[1][0][1]} - @{users_data[1][0][3]}'
        await deposit_repo.minus_user_deposit(users_data[0][0][0], rate[0])
        await deposit_repo.plus_user_deposit(users_data[1][0][0], rate[0])
        await stat_repo.update_win_blackjack(users_data[1][0][0])
        await stat_repo.update_games_blackjack(users_data[0][0][0])
        await stat_repo.update_games_blackjack(users_data[1][0][0])
    elif result == 'draw':
    ...
    for player in players:
        ...
        await bot.send_message(player[2], parse_mode=types.ParseMode.HTML, text=
        f"Игра окончена!\n\n"
        f"Рука {users_data[0][0][1]} - @{users_data[0][0][3]}: {players_hands[0]} - {await repo.total_up(players_hands[0])}\n"
        f"Рука игрока {users_data[1][0][1]} - @{users_data[1][0][3]}: {players_hands[1]} - {await repo.total_up(players_hands[1])}\n\n"
